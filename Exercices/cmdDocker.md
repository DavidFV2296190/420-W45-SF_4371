...
```bash
$docker container run --name test1 -it busybox
$docker container run -p 4000:80 friendlyhello          # Run "friendlyname" mapping port 4000 to 80
$docker container run -d -p 4000:80 friendlyhello        # Same thing, but in detached mode
$docker container run -p 3307:3306 -e MYSQL_ROOT_PASSWORD=Passw0rd -d --name MonMySQL  mysql:latest
